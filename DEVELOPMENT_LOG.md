...
- 参数区标题改为 `参数（建议不要动，除非你懂什么意思）`，英文同步为 `Settings (do not change unless you know what they mean)`。
- 点击开始剪辑前，在处理日志写入粗略预计耗时：
  - 根据视频时长、最多分析秒数、识别帧率和快速无损截取开关估算。
  - 提示导出时间取决于视频长度、电脑性能、识别帧率和导出设置。
  - 提醒用户处理时可以去喝杯茶，放松一下。
- 开始剪辑后按钮文案变为 `剪辑中`，任务结束后恢复 `开始剪辑`。

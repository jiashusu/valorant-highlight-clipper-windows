...
### Windows 版修复：误剪、弹窗、滚轮、窗口、画质和日志输入

- 问题来源：
  - 仍会剪入大量队友击杀画面。
  - 每次导出或生成预览时会弹系统命令窗口。
  - 鼠标滚轮不能滚动 Highlights 卡片区。
  - 默认窗口偏小。
  - 导出视频比原视频模糊。
  - 处理日志可以输入，不符合输出日志定位。
- 本次修改：
  - 本人击杀过滤进一步收紧：
    - 默认置信度从 `0.8` 提高到 `0.93`。
    - 严格过滤模式下底层最低置信度提高到 `0.94`。
    - 严格过滤模式下最短事件持续时间提高到 `0.75` 秒，短暂误触发会被过滤。
  - 导出画质改为优先清晰：
    - H.264 `preset=slow`。
    - `crf=14`。
    - `pix_fmt=yuv420p`。
    - 音频优先复制，失败时回退为 AAC 192k。
  - Windows 后台调用 `ffmpeg`、`ffprobe`、`ffplay` 时使用隐藏窗口参数，避免弹系统命令窗口。
  - 默认窗口改为 `1280x840`，最小窗口改为 `1120x740`。
  - Highlights Canvas 和卡片内子控件都绑定鼠标滚轮，鼠标停在缩略图、文字或按钮上也可以滚动。
  - 处理日志改为只读，只有程序可以写入；“清空日志”按钮仍可清空。
  - 保留 3 列高光卡片、卡片内低清预览、高清播放、打开目录和删除片段。

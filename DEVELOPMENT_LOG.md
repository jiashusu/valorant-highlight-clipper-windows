...
- 用户反馈：
  - 默认窗口还需要更大。
  - 原 UI 太像复古系统控件，希望黑色主题、按钮更有特色。
  - 检查更新需要修复。
  - 卡片内预览需要更流畅、更清晰。
  - 导出视频操作从“打开目录”改为直接定位该视频文件。
  - 继续优化流程，不删功能。
- 本次修改：
  ...
  - 使用 Tkinter `clam` 主题重做深色 UI：
    ...
    - 红色强调按钮。
    - 深色输入框、表格、日志、进度条。
  - 检查更新改为读取 Windows 仓库最新 Release：
    - `jiashusu/valorant-highlight-clipper-windows`
    - 发现新版时打开 Windows Release 页面，不再跳 macOS Actions。
    - 私有仓库自动检查失败时，可直接打开 Release 页面手动查看。
  - 卡片预览提升：
    - 缩略图/预览尺寸从 260x146 提升到 320x180。
    - 卡片内预览帧率从 20fps 提升到 30fps。
    - 预览帧和缩略图使用更高质量的 Lanczos 缩放，JPEG 质量提高。
